...
module top (
    ...
    input  wire [16:0] image_data,  //a displayed image
    output wire        hsync,
    output wire        vsync,
    ...
    // --------------------------------------------------
    wire [7:0] fb_x = x[9:2];   // 0-319
    wire [7:0] fb_y = y[9:2];   // 0-239
...
1, Use python code.
If you want to implement this function using python, you first run the python code on your computer to convert the image and then store it in the FPGA's SRAM.
```
